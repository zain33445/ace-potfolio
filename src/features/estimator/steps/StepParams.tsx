...
export default function StepParams({ inputs, setInputs }: { inputs: Inputs; setInputs: (updater: (prev: Inputs) => Inputs) => void }) {
  return (
    <div className="space-y-4">
      <div>
        <h4 className="font-mono text-sm text-primary tracking-widest uppercase font-bold">Step 3 of 4</h4>
        <p className="font-space text-xl text-on-background mt-1">Refine the parameters</p>
        <p className="font-mono text-xs text-on-surface-variant mt-1">Fine-tune complexity, delivery speed, and regional cost factors for a more precise estimate.</p>
      </div>
          ...
          <div className="flex border border-blueprint-line p-1 bg-background bracket-corners">
            {(['simple', 'medium', 'high'] as const).map((level) => (
              <button
                ...
                className={`flex-1 py-1.5 font-mono text-xs uppercase font-bold transition-all duration-200 ${
      ...
      <div className="space-y-2">
        ...
        <div className="relative">
          <input
            ...
            className="w-full bg-background border border-blueprint-line focus:border-primary focus:ring-0 px-4 py-3 font-mono text-base tracking-widest outline-none transition-colors"
            placeholder="75001"
          />
          <span className="absolute right-3 top-3.5 font-mono text-xs text-primary opacity-60">
