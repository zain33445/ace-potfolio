...
export interface OptionWheelHandle {
  /** Drive the wheel to a fractional position (e.g. from page-scroll scrubbing). */
  setPosition(value: number): void;
  /** True while the user is actively dragging the wheel. */
  isDragging(): boolean;
}
...
export interface OptionWheelProps {
  ...
  /** Custom per-option content (e.g. a numbered prefix); falls back to the raw label. */
  renderItem?: (index: number, item: string) => ReactNode;
  textColor?: string;
  activeColor?: string;
  /** Font weight applied to the selected option. */
  ...
  loop?: boolean;
  draggable?: boolean;
  /** When false, mouse-wheel input over the wheel scrolls the page normally. */
  captureWheel?: boolean;
  ref?: Ref<OptionWheelHandle>;
  soundUrl?: string;
  soundVolume?: number;
  className?: string;
  /** Accessible name for the wheel (defaults to "Options"). */
  ariaLabel?: string;
}
...
const OptionWheel = ({
  ...
  const playTick = useCallback(() => {
    ...
    if (!soundUrl || soundVolume <= 0) return;
    ...
    try {
      ...
      // Audio playback not supported or blocked — silently ignore.
    }
  }, []);
  ...
  // Opt-out via `captureWheel` for scroll-driven usages where the wheel must
  // not trap page scrolling.
  useEffect(() => {
  ...
  // External scrubbing (e.g. page-scroll-driven steps). Ignored mid-drag so
  // an in-flight scroll update can't yank the wheel out of the user's hand.
  ...
  // Unlock audio playback on first user interaction so sounds work even
  // though the wheel is primarily driven by page scroll.
  useEffect(() => {
    const el = rootRef.current;
    if (!el) return;
    let unlocked = false;
  ...
  return (
    <div
      ...
      className={`relative h-full w-full font-thin select-none overflow-hidden outline-none [touch-action:pan-y] ${isDragging ? "cursor-grabbing" : "cursor-grab"}${className ? ` ${className}` : ""}`}
      style={
        {
          "--ow-text-color": textColor,
          "--ow-active-color": activeColor,
          "--ow-font-size": `${fontSize}rem`,
          "--ow-font-weight": activeFontWeight,
          "--ow-inset": `${inset}px`,
        } as CSSProperties
      }
    ...
    >
      {items.map((label, index) => (
        <div
          ...
          } ${selectedIndex === index ? "[font-weight:var(--ow-font-weight,400)]  border-b-4 border-primary" : ""}`}
          onClick={() => handleItemClick(index)}
        >
          {renderItem ? renderItem(index, label) : label}
        </div>
      ))}
    </div>
  );
